    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <h2 className="text-4xl lg:text-6xl font-bold text-gray-900 mb-4 bg-gradient-to-r from-gray-900 to-orange-600 bg-clip-text text-transparent">
        ...
        <div
          className={`mb-12 transition-all duration-1000 delay-300 ${isVisible ? "translate-x-0 opacity-100" : "-translate-x-10 opacity-0"}`}
        >
          <div ref={timelineRef} className="flex space-x-4 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory">
            {processSteps.map((step, index) => {
              const IconComponent = step.icon
              return (
                <div
                  key={index}
                  className={`flex-shrink-0 snap-start cursor-pointer transition-all duration-300 ${index === activeStep ? "scale-100" : "scale-95 opacity-70"}`}
                  onClick={() => scrollToStep(index)}
                >
                  <div
                    className={`w-24 h-24 rounded-full flex items-center justify-center bg-gradient-to-br ${step.color} ${index === activeStep ? "ring-4 ring-orange-500 ring-offset-4" : ""}`}
                  >
                    <IconComponent className="h-10 w-10 text-white" />
                  </div>
                  <div className="text-center mt-2 text-sm font-medium">
                    <span className="bg-gray-200 text-gray-800 rounded-full w-6 h-6 inline-flex items-center justify-center mb-1">
                      {index + 1}
                    </span>
          ...
          <div className="h-1 bg-gray-200 rounded-full w-full mt-2 mb-8 relative">
            <div
              className="absolute h-1 bg-gradient-to-r from-orange-500 to-orange-600 rounded-full transition-all duration-500"
        ...
        {/* Active Step Details */}
        <div
          className={`grid lg:grid-cols-2 gap-12 items-center transition-all duration-500 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
          style={{ transitionDelay: "500ms" }}
        >
          <div className="order-2 lg:order-1">
            <div
              className={`bg-gradient-to-br ${processSteps[activeStep].color} text-white w-16 h-16 rounded-2xl flex items-center justify-center mb-6`}
            >
              {(() => {
            ...
            <h3 className="text-3xl font-bold text-gray-900 mb-4 flex items-center">
              <span className="bg-orange-100 text-orange-600 rounded-full w-8 h-8 inline-flex items-center justify-center text-sm mr-3">
                {activeStep + 1}
              </span>
              {processSteps[activeStep].title}
            </h3>
            <p className="text-lg text-gray-600 mb-8 leading-relaxed">{processSteps[activeStep].description}</p>
            ...
            <div className="flex space-x-4">
              <Button
                variant="outline"
                className={`border-orange-200 text-orange-600 hover:bg-orange-50 ${activeStep === 0 ? "opacity-50 cursor-not-allowed" : ""}`}
                onClick={prevStep}
              ...
              <Button
                className={`bg-orange-500 hover:bg-orange-600 text-white ${activeStep === processSteps.length - 1 ? "opacity-50 cursor-not-allowed" : ""}`}
                onClick={nextStep}
          ...
          <div className="order-1 lg:order-2 relative">
            ...
            <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
              {processSteps.map((_, index) => (
                <button
                  key={index}
                  className={`w-3 h-3 rounded-full transition-all duration-300 ${index === activeStep ? "bg-orange-500 w-6" : "bg-gray-300 hover:bg-gray-400"}`}
                  onClick={() => scrollToStep(index)}
        ...
        {/* Call to Action */}
        <div
          className={`mt-20 text-center transition-all duration-1000 delay-700 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}
        >
          <Card className="bg-gradient-to-br from-orange-500 to-orange-600 text-white border-0 shadow-xl">
            <CardContent className="p-8">
              ...
              <h3 className="text-3xl font-bold mb-4">Ready to Start Your Construction Journey?</h3>
              <p className="text-lg text-white/90 max-w-2xl mx-auto mb-8">
                Our proven process ensures your project is completed on time, within budget, and exceeds your
                expectations.
              </p>
